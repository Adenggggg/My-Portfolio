...
function VideoModal({ video, onClose }: { video: Video; onClose: () => void }) {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(0,0,0,0.85)", backdropFilter: "blur(12px)", animation: "fadeIn 0.2s ease both" }}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-4xl overflow-hidden rounded-2xl shadow-2xl"
        style={{ border: "1px solid var(--border-col)", background: "var(--bg-card)", animation: "slideUp 0.28s cubic-bezier(0.22,1,0.36,1) both" }}
      >
        ...
        <div className="flex items-start justify-between gap-4 px-6 py-5">
          <div>
            {video.tag && (
              <p className="mb-1 font-mono text-[10px] uppercase tracking-widest" style={{ color: "var(--text-3)" }}>
                {video.tag}
              </p>
            )}
            <p className="text-lg font-semibold" style={{ color: "var(--text-1)" }}>{video.title}</p>
            {video.description && (
              <p className="mt-1.5 max-w-xl text-sm leading-relaxed" style={{ color: "var(--text-2)" }}>
                {video.description}
              </p>
            )}
          </div>
    ...
    <div className="cursor-pointer" onClick={() => onOpen(video)}>
      ...
      >
        <video
          ...
          style={{
            filter: playing ? "brightness(0.75)" : "brightness(0.45)",
        ...
        {video.tag && (
          <div
            className="absolute left-3.5 top-3.5 rounded-full px-2.5 py-1 font-mono text-[10px] uppercase tracking-widest backdrop-blur-md"
            style={{ border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.5)" }}
          >
            {video.tag}
          </div>
        )}
        ...
        <div className={`absolute bottom-3.5 right-3.5 transition-opacity duration-300 ${playing ? "opacity-100" : "opacity-0"}`}>
          <button
            ...
            style={{ border: "1px solid rgba(255,255,255,0.15)", background: "rgba(0,0,0,0.5)", color: "white" }}
          >
            {muted ? (
          ...
          <div className="fade-up delay-1 mb-14">
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <p className="text-sm" style={{ color: "var(--text-3)" }}>
                Commercial Ads, Cinematics, and Edits.
              </p>
            </div>
          </div>
